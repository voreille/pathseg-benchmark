...
class TwoHeadSemantic(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if out.get("logits_b_base") is not None:
            ...
            if logits_b_base.shape[-2:] != self.img_size:
                logits_b_base = F.interpolate(
                    logits_b_base,
                    self.img_size,
                    mode="bilinear",
                )
            loss_b_base = self._loss_on_subset(
                logits_b_base, targets, m_b, self.criterion_b
            )
            self.log("train_loss_b_base", loss_b_base, sync_dist=True, prog_bar=False)
            loss_b = loss_b + 0.3 * loss_b_base
