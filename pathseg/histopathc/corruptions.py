# The code is modified version of the original imagecorruption library (pip).
...
def impulse_noise(x, severity=1):
    c = [.03, .06, .09, 0.17, 0.27][severity - 1]
    salt_vs_pepper = 0.5  # Default value in skimage
    ...
    salted = np.random.rand(*x.shape) <= salt_vs_pepper
    peppered = ~salted
